        ...
        DetectFormat(string rawText, string[] lines, List<string> nonEmptyLines, bool? hasHeaders = null)
    {
        ...
        if ((trimmed.StartsWith('{') && trimmed.EndsWith('}')) || (trimmed.StartsWith('[') && trimmed.EndsWith(']')))
        {
            try
            {
                ...
                {
                    // Check if array of objects (tabular JSON)
                    var items = doc.RootElement.EnumerateArray().Take(5).ToList();
                    if (items.Count > 0 && items.All(x => x.ValueKind == JsonValueKind.Object))
                    {
                        var headers = items[0].EnumerateObject().Select(p => p.Name).ToList();
                        int totalRows = doc.RootElement.GetArrayLength();
                        return (DetectedFormat.Json, $"JSON Array of Objects ({totalRows} items, {headers.Count} properties)", null, true, headers.Count, totalRows, headers, true);
                    }
                    return (DetectedFormat.Json, $"JSON Array ({doc.RootElement.GetArrayLength()} items)", null, false, 0, 0, Array.Empty<string>(), true);
                }
                return (DetectedFormat.Json, "JSON Object", null, false, 0, 0, Array.Empty<string>(), true);
            }
            catch
            {
