...
public class FileLoadingTests : IDisposable
{
    ...
    {
        _tempDirectory = Path.Combine(Path.GetTempPath(), "TextForgeTests_" + Guid.NewGuid().ToString("N"));
    ...
    public void LoadFromFile_ValidJsonFile_BeautifiesJsonContent()
    {
        ...
        string rawJson = "{\"name\":\"TextForge\",\"version\":1}";
        ...
        Assert.True(result);
        Assert.Contains("\"name\": \"TextForge\"", vm.InputText);
