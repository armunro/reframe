...
";
    ...
    {
        string json = "{\"name\": \"TextForge\", \"version\": 1, \"features\": [\"Lines\", \"Tables\"]}";
        string yaml = DeveloperTransformers.JsonToYaml(json);

        Assert.Contains("name: TextForge", yaml);
...
";
        ...
        Assert.Contains("\"name\":\"TextForge\"", json);
...
";
    ...
    {
        var vm = new MainViewModel();
        vm.InputText = "{\"app\":\"TextForge\",\"stars\":100}";

        vm.ActionCommand.Execute("JsonToYaml");
        Assert.Contains("app: TextForge", vm.OutputText);
